...
The smoke test requires a local `shuck` executable and verifies the native server's advertised diagnostics/formatting capabilities, a real diagnostic, and a formatting edit. The final command exercises the equivalent `shuck.toml` configuration filename; exit status 1 is expected because `--diff` reports the pending formatting change. Load the repository through Zed's **Install Dev Extension** action to test the same integration from the editor.
